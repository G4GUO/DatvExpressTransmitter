...
extern uint8_t sstd[4][SYMS_IN_FRAME];
extern DVBTFormat m_format;

FFTComplex rt_2k[SYMS_IN_FRAME*SF_NR][M2KS];
int        dt_2k[SYMS_IN_FRAME*SF_NR][M2KS];
FFTComplex rt_8k[SYMS_IN_FRAME*SF_NR][M8KS];
...
void build_2k_sf_ref( void )
{
    int l,k,i,p,s;
    // Zap everything
    memset( rt_2k, 0,sizeof(FFTComplex)*SYMS_IN_FRAME*SF_NR*M2KS);
...
void build_8k_sf_ref( void )
{
    int l,k,i,p,s;
    // Zap everything
    memset( rt_8k, 0,sizeof(FFTComplex)*SYMS_IN_FRAME*SF_NR*M8KS);
