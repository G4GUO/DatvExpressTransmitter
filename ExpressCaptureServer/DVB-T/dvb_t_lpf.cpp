...
FFTComplex m_mem[FILTER_TAP_NUM];// Circular buffer
FFTComplex *m_out = NULL;
int m_ip;
int m_length = 0;
...
FFTComplex *dvbt_filter( FFTComplex *in, int length )
{
    ...
    if( length > m_length )
    {
        ...
        m_length = sizeof(FFTComplex)*length;
        m_out = (FFTComplex*)malloc(m_length);
    ...
    for( int i = 0; i < length; i++ )
    {
        ...
        {
            m_out[op].re += (float)filter_taps[n]*m_mem[k].re;
            m_out[op].im += (float)filter_taps[n]*m_mem[k].im;
