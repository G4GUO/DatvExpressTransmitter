...
void dvb_t_bch_encode( uint8_t *b )
{
    int i;
    long shift = 0;
    //
...
//
void build_diff_tp_tables( uint8_t *in, uint8_t *out )
{
    int i;
...
void build_tp_block( void )
{
	...
	for( i = 40; i < 54; i++ )
	{
        ...
        memcpy(s[1],s[0],sizeof(uint8_t)*SYMS_IN_FRAME);
        memcpy(s[2],s[0],sizeof(uint8_t)*SYMS_IN_FRAME);
        memcpy(s[3],s[0],sizeof(uint8_t)*SYMS_IN_FRAME);
        // Sync sequences
        s[0][1]  = 0; s[1][1]  = 1;
        s[0][2]  = 0; s[1][2]  = 1;
        s[0][3]  = 1; s[1][3]  = 0;
        ...
        s[0][15] = 1; s[1][15] = 0;
        s[0][16] = 0; s[1][16] = 1;
        memcpy(&s[2][1],&s[0][1],sizeof(uint8_t)*16);
        memcpy(&s[3][1],&s[1][1],sizeof(uint8_t)*16);
        //
