...
int f_ts_info( uint8_t *b, transport_stream_info *p )
{
    int len,i,pos,start;
...
int f_nit( uint8_t *b, network_information_section *p )
{	
    int i,len,pos,start;
    b[0] = 0x40;
    b[1] = 0;
...
void nit_fmt( int nit_pid, int stream_id, int network_id, int service_id )
{
    ...
    nis.ts_info[0].transport_stream_id = stream_id;
    nis.ts_info[0].original_network_id = network_id;
    ...
    nis.ts_info[0].desc[0].sld.entry[0].service_id   = service_id;
